...
description: >-
  ...
  or editing a skill, or modifying AGENTS.md or CLAUDE.md.
  Reference for the levers that make an agent-facing document predictable.
---

Reference for writing any document an agent consumes — a skill, a context file (`AGENTS.md`, `CLAUDE.md`), a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable — the agent taking the same _process_ every run, not producing the same output.
